...
void doGenerate(const unsigned int &inId, const unsigned int &inInputValue, const unsigned int &inIterations, const unsigned int &inDelay, std::atomic<unsigned int> &inResult)
{
    for(unsigned int i = 0; i < inIterations; i++)
    {
        if(inResult.load() != maxNumber + 1) //когда число найдено - прекращаем цикл
            break;
        ...
        if(v == inInputValue) //если числа совпадают
        {
            ...
            if(inResult.load() == maxNumber+1) //еще раз проверяем, ибо пока шли до lock'а могли уже поступить записи
            {
                inResult.store(i); //пишем
            }
...
int main()
{
    ...
    std::atomic<unsigned int> result(maxNumber + 1); //делаем изначальный результат недостижимым. Можно было сделать при помощи bool делать проверку, но тогда на один параметр больше бы стало
    ...
    if(result.load() < maxNumber) //проверка на корректное нахождение результата
        std::cout << "Победитель: " << result.load() << std::endl;
    else
